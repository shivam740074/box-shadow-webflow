...
const distanceSlider = document.getElementById('distance-slider') as HTMLInputElement | null;
const intensitySlider = document.getElementById('intensity-slider') as HTMLInputElement | null;
const sharpnessSlider = document.getElementById('sharpness-slider') as HTMLInputElement | null;
const colorPicker = document.getElementById('color-picker') as HTMLInputElement | null;
const livePreviewBox = document.getElementById('live-preview-box') as HTMLElement | null;
const applyCustomBtn = document.getElementById('apply-custom-btn') as HTMLElement | null;
...
const presetsSection = document.getElementById('presets-section') as HTMLElement | null;
const showPresetsBtn = document.getElementById('toggle-presets-btn') as HTMLElement | null;
const backToControlsBtn = document.getElementById('back-to-controls-btn') as HTMLElement | null;

/**
 * Shows the custom controls panel and hides the presets panel.
 */
function showControlsPanel(): void {
 ...
 */
async function showPresetsPanel(): Promise<void> {
    if (controlsSection && presetsSection) {
...
async function initApp(): Promise<void> {
    ...
    if (applyCustomBtn) {
        applyCustomBtn.addEventListener('click', async () => {
            ...
            if (el && livePreviewBox) {
                await applyShadowToElement(el, livePreviewBox.style.boxShadow);
            } else if (!el) {
                flashMessage("Please select an element first.", "error");
            } else {
    ...
    const initialElement = await getSelectedElement();
    if (initialElement) {
    ...
    try {
        if (typeof wf.on === 'function') {
            try { wf.on('ready', updateUIBasedOnSelection); console.log("[box-shadow] Webflow 'ready' listener attached."); } catch (e) { console.error("[box-shadow] Failed to attach 'ready' listener", e); }
            try { wf.on('selectionchange', updateUIBasedOnSelection); console.log("[box-shadow] Webflow 'selectionchange' listener attached."); } catch (e) { console.error("[box-shadow] Failed to attach 'selectionchange' listener", e); }
        }
    } catch (e) { console.error("[box-shadow] Error attaching Webflow event listeners", e); }
